...
#### 単位 11 の未計測: Lean に SHA-256 が無い

Lean core にも Std にも無い（実測）。選択肢は **Lean で書く**（依存が増えない）か
**C で `csrc/` に足す**（`vendor/md4c` と同じ経路。ただし第三者コードなら帰属表示の義務）。
**まず Lean で書いて計測する** — 判断のしきい値は
「e2e/micro の 11 モジュールで 2 秒 / 対象の 432 モジュールで 60 秒」。
Rust は concurrency 8 で並列、Lean は順次（計画で「スレッド化は入れない」）なので
**数倍遅いのは織り込み済み**。効くのは消費者が待つ絶対値。
