...
#   2 CLANG     the C was compiled by something other than the toolchain's own
#               clang. The build is green either way — that is the point: a bare
#               `cc` builds here and fails on a consumer with no C toolchain.
#   3 NO LEAN   a module under `src/` imports `Lean`. An executable that does
...
#
# **Two items left with `crates/` and were not replaced.** One compared
# `--version` byte for byte against the Rust binary; the other reconciled
# `src/Litedoc4/Version.lean` against `Cargo.toml`'s workspace version. There is
# now one place the version is written, so there is nothing to reconcile it
# against. "Does `--version` print the literal" was tried and is **not** here:
# item 1 rebuilds from that literal, so the answer is yes by construction and
# the item could not fail (measured 2026-09-02 — a 9.9.9 literal passed it).
#
# Item 2 reads `lake build -v`, not the exit code: `LITEDOC4_SYSTEM_CC=1` builds
...
say "1/3 the Lean half builds from a consumer's workspace"
built=0
...
say "2/3 the C was compiled by the toolchain's own clang"
...
if [ "$prefix_rc" -ne 0 ]; then
  fail 2 "lake env lean --print-prefix failed in $FIXTURE — the toolchain cannot be identified; see $OUT/prefix.err"
else
  PREFIX="$(tr -d '\n' <"$OUT/prefix.txt")"
...
PY
    ...
    fail 2 "lake build -v of the C targets failed — see $OUT/cc.log"
    tail -20 "$OUT/cc.log" >&2
  fi
fi

say "3/3 nothing under src/ imports Lean"
lean_import_rc=0
...
echo
echo "PURELEAN GATE: ok — $ITEMS items; the two version items retired with crates/ and have no successor"
