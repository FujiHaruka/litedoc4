...
The chain is written out here rather than cited, because the file it was once
cited from is not in the tree:
...
Being the same bytes is the point. doc-gen4 parses docstrings with this md4c and
the Lean half compiled from this directory parses them with this md4c — so the
two agree by construction rather than by version number.
...
pages against the Rust half as it was when that comparison was taken.
...
and the renderer builds the HTML itself, the same division doc-gen4 uses.
`md4c.c` includes only `md4c.h` and standard headers, so the parser builds
without them.

## Updating

This directory is the origin, so an update means a new md4c release. Record the
release here and re-run the 422-page comparison against the frozen output
(`benchmarks/results/purelean-md4c-2026-08-30.txt`) before taking it.
