...
- **足すときの罠**: `rg '#\[ignore'` は **doc comment 中の「Not `#[ignore]`:」も拾う**
  (`crates/litedoc4-global/tests/web_fixture.rs:207` に実在)。
  `--verify-list` が同じ素朴な grep で数えているなら、**doc に `#[ignore]` と書いた日に
  台帳が偽の不一致で赤くなる** — ゲート自身が「文字列」と「属性」を取り違える形。
  **先に `corpus-gate.sh` が今どう数えているかを読む**
...
- 直し方: 3 つの doc を実測に合わせる (「ドットが消えるから届かない」→
  ...
  render 側にガードを入れるかは段 3 の X4 で判断する
...
- **D1 で土台ができた**【2026-08-23、`bdad7d2` の次】: `main.rs` の
  `const LEDGER_FLAGS: [(&str, &[&str]); 17]` が「どのフラグをどのサブコマンドが受けるか」を持つ。
  R9 はこれを USAGE と突き合わせる。**ただし今は `LEDGER_FLAGS` と `match` のアームの一致を
  誰も検査していない** — `match` に足してテーブルに足し忘れると、そのフラグは
  **3 つとも受理する元の挙動に戻る**。R9 で塞ぐ
