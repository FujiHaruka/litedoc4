...
#   --litedoc4-bin <path>   the litedoc4 executable (default: <repo>/.lake/build/
#                           bin/litedoc4, built with Lake if missing)
...
DEFAULT_LITEDOC4_BIN="$REPO/.lake/build/bin/litedoc4"
LITEDOC4_BIN="$DEFAULT_LITEDOC4_BIN"
TIMINGS=""
BUILD_ARGS=()
...
step "4/5  the litedoc4 executable (Lean)"
t="$(now)"
# **Always ask Lake**, rather than skipping because the file is there. That is
...
#
# **The workspace is written here** because Lake cannot run beside litedoc4's own
# lakefile — there is deliberately no `lean-toolchain` there, and adding one
# would rewrite the consumer's (lakefile.lean says why). So the executable is
# built the way a consumer builds it: from a package that requires it. The
# `lean-toolchain` is copied from --root, so this never asks elan for a second
# Lean; it lives under `<repo>/.lake` so that one cache path holds both the
# generated workspace and the build output it produces, which have to come back
# together or Lake re-resolves and rebuilds.
#
...
if [ "$LITEDOC4_BIN" = "$DEFAULT_LITEDOC4_BIN" ]; then
  HOST="$REPO/.lake/host"
  mkdir -p "$HOST"
  if [ -f "$ROOT/lean-toolchain" ]; then
    cp "$ROOT/lean-toolchain" "$HOST/lean-toolchain"
  fi
  cat > "$HOST/lakefile.toml" <<TOML
# Generated by tools/ci-build.sh. The only package here is the `require` below:
# it exists so that Lake has a root to build litedoc4 from.
name = "litedoc4host"
...
path = "$REPO"
TOML
  (cd "$HOST" && "$LAKE" build litedoc4/litedoc4)
  [ -x "$LITEDOC4_BIN" ] || {
    echo "lake build litedoc4/litedoc4 exited 0 but there is no $LITEDOC4_BIN" >&2; exit 1; }
  record lean "$(elapsed "$t" "$(now)")" "built"
elif [ -x "$LITEDOC4_BIN" ]; then
  echo "given: $LITEDOC4_BIN (--litedoc4-bin, taken as it is)"
  record lean "$(elapsed "$t" "$(now)")" "given"
else
  echo "no litedoc4 executable at $LITEDOC4_BIN" >&2
  exit 1
fi
