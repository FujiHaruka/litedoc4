//! The declaration headers, declaration blocks and page frames, against
//! `fixtures/render/page-parts-expected.json`.
//!
//! The fixture was generated by `tests/oracle/gen-page-parts-expected.ts`,
//! which sliced `declHeader`, `declHtml`, `headHtml`, `pageHeaderHtml` and
//! `internalNavHtml` out of the prototype and ran them over the **real** IR
//! with the **real** `.lidx`. **Its `html` half is no longer an independent
//! oracle**: this renderer gives every declaration a `Used by` block the
//! prototype could not have had, so that half is this renderer's own output
//! re-frozen and [`bless`] regenerates it. The `header` half is still the
//! prototype's and is never blessed.
//!
//! **The oracle cannot be doc-gen4**: `docInfoToHtml` walks `DocInfo`s built
//! from a live environment, and the IR carries a flattened, already-printed
//! form of them, so there is no input both implementations could be handed.
//!
//! **The frame and header comparisons are over content, not bytes**, because
//! the markup was rewritten on purpose: what survives is everything the
//! prototype's bytes *said* rather than how they said it — the `id` attributes
//! in order (minus the instance stubs', which are keyed off `data-name` and
//! compared as [`Content::stubs`]), the `href` attributes in order, and the
//! words with the tags stripped as a **multiset**. Two of the three are
//! ordered, which keeps this from degenerating into "the same pieces,
//! somewhere"; the words are not, because the head reordered and comparing them
//! in order would only assert that reordering. **This is weaker than a byte
//! comparison** — a class name, an element or an attribute can move without
//! failing here — and it is the strength an oracle has once the two sides are
//! not trying to be the same document.
//!
...
const FIXTURE: &str = include_str!("../../../fixtures/render/page-parts-expected.json");

#[derive(Deserialize)]
#[serde(rename_all = "camelCase")]
struct Expected {
    oracle: String,
    /// The revision prefix the source links were built with, which has to be
    ...
    source_url: String,
    ir_modules: usize,
    /// The `declHeader` denominator.
    ir_declarations: usize,
    /// The `declHtml` denominator: declarations that get a page entry.
...
struct Frame {
    ...
    head: String,
    /// The prototype's `pageHeaderHtml`. Only two things in it survive here:
...
impl Case {
    ...
    /// The head and the signature, which together are what the prototype's
    /// `declHeader` was: this renderer splits them because they wrap
    /// differently, and moves the source link out of the block's own
    /// `div.gh_link` into the head.
...
fn expected() -> Expected {
    serde_json::from_str(FIXTURE).expect("fixtures/render/page-parts-expected.json is valid")
}
...
/// declaration's member, collected **across every module**. Collecting it per
/// module leaves other modules' projections and constructors on the page.
#[test]
...
/// Rewrites `fixtures/render/page-parts-expected.json` from this renderer's output,
...
/// it is not a review. What changed, and because of what, is read out of this
/// output instead.
///
/// **Only `html` is rewritten.** `header` is the same fixture's record of the
/// prototype's head and signature; blessing that as well would make its half of
...
fn bless(e: &Expected) {
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../fixtures/render/page-parts-expected.json");
...
fn carries_the_same_content_as_the_frozen_output() {
    ...
    for case in &e.cases {
        ...
        match case.render() {
            Ok(html) => {
                // Byte for byte, because both sides are this renderer's:
                // `Content` bridges two different markups, and a bridge between
                // a thing and itself only loses information. The counters below
                // still come from `Content`, because what they guard is that
                // the *sample* has not shrunk.
                if html != case.html {
    ...
    // stopped emitting things cannot pass by being uniformly empty. The bounds
...
/// five, so they are matched by what they carry — the title, the search form,
/// the module's source link, the import list, the `imported-by` stub and one
/// entry per page declaration — rather than one to one.
///
/// `page_root` and `module_source_url` are still compared byte for byte: they
/// are pure functions of a module name, and they decide where every other link
/// on the page points.
#[test]
fn frames_carry_the_same_content_as_the_prototype() {
    ...
    for frame in &e.frames {
        ...
        // outside the page's own root — which the prototype's did not, since it
        // pulled four scripts off a CDN.
        ...
        // The page still spells out which module it is: the prototype's
        // `h2.header_filename` is the `<h1>` of the content column here, and
        // the spelling — one `span.name` per component — is
        // `litedoc4_render::break_within`'s on both sides.
        ...
        // hrefs. The prototype's links are all relative, which the empty
        // `ExternalLinks` reproduces, and it wrote a page for every module it
        // named — so this frame's world is one where each import has a page.
...
/// into one that a renderer emitting only headers would pass. Every probe reads
/// the **fixture's own bytes** and has to match at least one case.
#[test]
fn the_sample_reaches_every_shape() {
    ...
    // `kindDescription` and collapsing the two is the obvious mistake.
...
/// against the real IR and the real `.lidx`. The only place the whole-run
/// inputs are built the way a run builds them — the name index from the
/// dependency slices, the modules and the `.lidx`; `suppressed` from **every**
/// module — so it is what stands behind the claim that the reduced worlds above
/// are the same thing in miniature.
#[test]
...
fn the_whole_corpus_carries_the_prototypes_content() {
    ...
    // environment, so its relative links into `Mathlib.*` point at pages it
    // wrote — a run's world has pages for the target package alone.
    let index = builder.build_with_a_page_for_every_module(links, ExternalLinks::default());
    let code = CodeRenderer::new(&index);

    // Across **every** module, which is why this cannot be folded into the
    // per-module loop below.
...
/// What a fragment of HTML *says*, as opposed to how it says it. Each of the
/// four lists is compared the strongest way it can be: the anchors and the
/// links in document order, the rest as sorted multisets. The file heading says
/// why this and not the bytes.
#[derive(Debug)]
struct Content {
...
impl Content {
    fn of(html: &str) -> Self {
        ...
        // The one place the rewrite changed what a block *says* rather than
        // how: a named constructor was `Name :: ( … )` wrapped around the field
        // list and is now `constructor Name` above it. Both spellings come out,
        // and the constructor's name is still compared — it is the field list's
        // `id` on either side.
    ...
    /// A whole declaration block against the prototype's. The head's two hrefs
    /// are swapped and nothing else moved: doc-gen4 put the source link in a
    /// `div.gh_link` **above** the block and this crate puts it in the head,
    /// after the self link.
    ...
    /// is the same content **plus** the source link this renderer moved into
    /// it.
