...
# the oracle for the Lean port, and that binary left with `crates/` — it is
# `git show rust-frozen:crates/litedoc4/src/main.rs` and the tree around it, in
# that tag and not in HEAD. Refusal behaviour is the one class that needs no
# oracle once its answer is written down, and this is where it is written down.
#
...
#
# **One arm, and it used to be two.** The second asked whether the frozen answer
# was still what the *oracle* said, and it retired with `crates/`; the summary
# line names it as retired rather than leaving the reader to infer that a
# one-armed run is the whole gate. Rows the two halves worded differently by
# design carry `rust-differs` and are counted in that line, so the record of
# where they parted does not become an unread field.
#
# The arm counts how many cases reported a result and fails if that is not the
...
#
# **One normaliser.** A refusal that names a path may name an absolute one, and
# the directory it is under differs per run and per machine; `normalise`
# replaces it (and its realpath — `/var` is a symlink on macOS and `/tmp` is one
# too) with `<dir>`. It was one because two arms had to compare the same thing;
# it stays one because the frozen bytes were normalised by it.
#
...
# own. Sharing one directory would be cheaper by one `mkdir`, and it is not done
# because a case that writes into its own cwd — `build` does — would hand the
# next case a tree the previous one left behind.
#
# **There is no `--mint`, and that is the point.** The rows were minted from the
# Rust half; re-minting them from the Lean half would replace the answer with
# whatever the thing under test does today, which is the question, not the
# answer. A row that has to change is changed by hand, by someone who has read
# why it moved.
#
# usage: refusal-gate.sh [--lean PATH] [--out DIR] [--keep]
...
rc=0
python3 - "$EXPECT" "$ON_DISK" "$OUT" "$LEAN" <<'PY' || rc=$?
import os
import pathlib
import shlex
import shutil
...
def parse(path, cases, names, usage_lines, fresh):
    ...
    file_body = None
    lead = []
    # Everything before the first record is the file's header. The first
    # record's `lead` is dropped rather than carried, so that a rewriter cannot
    # write the header's last comment block twice.
    started = False
...
reported, lean_problems, diffs = arm("lean", lean, usage, cases)
problems += lean_problems
if diffs:
    (out / "lean-diff.txt").write_text("\n".join(diffs), encoding="utf-8")

# The rows the retired oracle worded differently. Read here so that the field
# stays one the gate accounts for: a declaration nothing counts is a comment
# with a syntax.
differs = sorted(case["name"] for case in cases if case["rust-differs"])

if len(reported) != declared:
    missing = sorted(set(case["name"] for case in cases) - set(reported))
    problems.insert(0, (
        f"lean arm reported {len(reported)} result(s) for {declared} `@case` row(s)"
...
if len(problems) > len(shown):
    print(f"REFUSAL GATE FAIL  and {len(problems) - len(shown)} more; the whole of every "
          f"difference is in {out}/lean-diff.txt", file=sys.stderr)
if problems:
    sys.exit(1)

printing = sum(1 for case in cases if case["stdout-not-frozen"])
print(f"REFUSAL GATE: lean {len(reported)}/{declared}"
      + "; the oracle arm is retired — the frozen answers were minted from the "
      + "Rust litedoc4 at tag rust-frozen and nothing re-asks it"
      + (f", including the {len(differs)} row(s) it worded differently "
         "(`grep -c rust-differs tools/refusals*.txt`)" if differs else "")
      + f"; usage block {len(usage.splitlines())} line(s)"
