...
# is also what `.github/workflows/ci-lean-versions.yml` runs on every toolchain
# in `tools/lean-toolchains.txt`. One script, because "which Lean compiled
# litedoc4, and from what workspace" is one question and two answers to it would
# drift.
#
...
if [ "$LITEDOC4_BIN" = "$DEFAULT_LITEDOC4_BIN" ]; then
  LAKE="$LAKE" "$REPO/tools/build-lean-exe.sh" --toolchain-from "$ROOT" --lake "$LAKE" >/dev/null
  [ -x "$LITEDOC4_BIN" ] || {
    echo "build-lean-exe.sh exited 0 but there is no $LITEDOC4_BIN" >&2; exit 1; }
