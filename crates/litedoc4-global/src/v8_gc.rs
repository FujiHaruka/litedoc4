...
//! code spans on.
//!
//! **Was generated by `tools/oracle/gen-v8-gc-table.ts`.** The ranges are the
//! answers of the V8 in `deno 2.7.14` (V8
//! `14.7.173.20-rusty`), enumerated over every code point. That script now
//! writes `src/Litedoc4/Global/V8GcTable.lean` and checks that one instead, so
//! nothing re-derives this copy; it leaves with the rest of `crates/`.
//!
