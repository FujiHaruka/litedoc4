...
#### 決定（2026-08-31、ユーザー判断）— (b) 足場を作り、部分移植してから削除

**不変条件は Lean でもコードとして表現しなければならない。**
**型で表現できるなら型で表現する。テストでしか表現できないものがあるなら、それはテストにする。**

この原則が移植の形を決める。**316 件の機械的な 1:1 翻訳ではない** —
不変条件ごとに「型・構造・名前で表現できるか」を先に問い、
できないものだけがテストになる。CLAUDE.md の「コメントが要ると感じたら、まず設計の匂いとして
読め。名前・分割・型・データ構造で同じことを言えないか先に試せ」と同じ判断を、
テストに対して行うということ。

順序: 足場 → 316 件のトリアージ（型に落ちるもの / テストが要るもの）→ 移植 → 削除。
