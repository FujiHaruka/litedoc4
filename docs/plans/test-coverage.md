...
**状態**: 進行中 (2026-08-24)。全 30 項目中 **決着 21**。**新しく書いたテストは 55 本**
(段 1 が 10、段 2 が 22、段 3 が 18、段 4 が 5)。**残り 9** — 段 6 (P1〜P5) と 段 7 (F1〜F4)。

**`cargo test --workspace` の報告は 503 → 562 (+59) で、書いた本数と 4 だけ食い違う。**
差は `tests/common/mod.rs` の 4 本で、**`tests/watch.rs` という新しいバイナリが `mod common;`
を宣言したので、同じ 4 本がそこでも走る**【実測 2026-08-24】。`cargo` の報告は
**走った回数**であって**書いた本数**ではない。**片方だけを「テストが N 本増えた」と
書かないこと** — バイナリを 1 つ足すたびに、書いていないテストの数が増える。
**決着のうち 6 件は「やることが無かった」** — 段 0 の 3 件は**測ったら既に CI で走っていて**、
段 5 の 3 件は**未カバー行が入力から到達できない防御分岐だった**。
**そして 7 件目が段 4 で出た** — `watch-gate.sh` は「計測対象と 171 MB の extractor が要る」
と書いていたが、**要らなかった**。`e2e/micro` に対して **数秒**で走る
【実測 2026-08-24 → `benchmarks/results/watch-gate-e2e-2026-08-24.txt`】。
...
| ID | やること |
|---|---|
| **F1** | カバレッジを**同じ手順で再計測**し、前後を `benchmarks/results/coverage-2026-08-24.txt` に書く。**`watch.rs` の残り約 73 行を「未検査」と書かない** — SIGKILL された子プロセスの profraw が落ちているだけで、その行は W2 が log で assert している (→ 段 4 の申し送り)。**cold/warm は無関係だが、母数 (走ったテスト数) は必ず記録する**。**測り終えたら `cargo llvm-cov clean` する — 掃除の主体はこの段** (`target/llvm-cov-target` は 880 MB あり、`--workspace` の再計測でほぼ倍になる。**このリポジトリは満杯のディスクで対象リポジトリを一度壊している**) |
