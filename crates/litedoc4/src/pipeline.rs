...
//! - **extract → ownership → merge is a loop**, bounded by `--max-rounds`; the
//!   bound reached with modules still stale is [`EXIT_ROUNDS`].
//! - **A moved `renderKey` overrides `--mode` with `all`** — the one page set that
//!   does not follow from any changed module.
//! - **`--jobs` is the resident extractor's start-time configuration**
//!   (`Extract.lean:2751`), so it is a flag of this command **only with
//!   `--serve`**, which is exactly when this command is the thing that starts the
//!   server. Behind `--extractor` the parallelism belongs to the extraction
...
//!   Snapshot it late and the delta compares the new map with itself: always
//!   empty, and every page that went stale through the global map is dropped
//!   without a word.
//!
//! The two halves of the render set — `ImpactRun::summary` and
//! `GlobalSummary::delta` — are unioned **in memory**. `global-set.txt`,
//! `impact-set.txt` and `render-set.txt` are written into `--work` as
//! diagnostics and read back by nothing, so a file that is missing, empty or
...
//! answer has already lost. It hands the ledger `detect` computed to
//! [`crate::build`] in [`Run::detected`], and that command writes it after the
//! last step that can fail. A chain of bare `incremental` runs therefore needs
//! `litedoc4 ledger build` between them, or the second run re-extracts the first
//! run's changed set again — wasteful, not wrong.
...
/// `--max-rounds` reached with modules still stale. The one number a caller may
/// branch on: it means "the loop did not converge", not "something is broken".
pub(crate) const EXIT_ROUNDS: u8 = 5;

/// The extractor exited non-zero.
///
/// **Deliberately not the child's own code**, which can be 5 — and 5 already
/// means [`EXIT_ROUNDS`] here. A caller that has to tell those apart cannot, so
/// the child's code goes in the message instead of on the process.
...
pub(crate) struct Incremental<'a> {
    ...
    /// and the full generation have to answer "what is this site called" the same
    /// way, and two readers is two answers.
    ...
    /// modules`. **Required**: without it `check` re-reads the ledger's own list
    /// and **cannot see a module that appeared or vanished**.
    pub modules: Vec<String>,
    pub source_url: &'a str,
    pub link_index: &'a Path,
    /// Where each **dependency's** source lives. Resolved by the caller, once,
    /// and used twice in here: `detect` hashes it into the render key and step 7
    /// renders with it. A round that resolved it a second time could disagree
    /// with the ledger it just checked.
...
/// in, an IR tree and a timings record out — and nothing downstream can tell
/// which one ran.
pub(crate) enum Extractor {
    /// `--extractor <program>`: a program called once per round.
    ///
    /// **There is no default**, and that is what makes this the seam: the
    /// contract is three flags, so anything that accepts them can be handed here.
    /// `litedoc4 extract` takes the same three, and the tests hand it a fake that
    /// copies a baked IR tree in — without which every test of this file would
    /// need a Lean toolchain and a 30-second extraction.
    OneShot {
        program: String,
...
impl Extractor {
    ...
    /// `<timings>-events.jsonl`, and the resident path reproduces that exactly so
    /// that two records of the same round stay comparable.
    pub(crate) fn run(
        &mut self,
        modules: &Path,
        ir_dir: &Path,
        ...
        // Counted before the exit code is judged: the run started an extractor
        // and paid for it whichever way it ended.
        *requests += 1;
        if status.success() {
            return Ok(());
        }
    ...
    ///
    /// A run over an unchanged package must answer **0**: it is the only work
    /// counter whose zero says Lean was never started.
    ...
    /// [`Resident`] closes the request pipe, which is what keeps the server alive
    /// at all. This exists so that the ordinary stop is reported and its cost is
    /// inside the run's clock, not so that a failure needs it.
...
/// The files one run leaves in `--work`. Every one is a **diagnostic**: the
/// pipeline writes them and never reads one back.
struct Work {
    changed: PathBuf,
    removed: PathBuf,
    render_all: PathBuf,
...
pub(crate) struct Summary {
    ...
    /// Pages the renderer **wrote** — **not the size of the render set**, which
    /// is the larger of the two whenever the set names a module the IR does not
    /// hold. The set's own size is on the `impact` line.
    ...
    /// the exit code — moves when a formula fails.
...
/// *licenses* but does not write (see the module heading).
...
pub(crate) fn run_incremental(
    ...
    // The dependency map's identity **before the rounds**, so that a rewrite one
    // of them performs can be seen. The ordinary case is that this run's own
    // extraction writes it.
    let map_before = link_index_digest(Some(options.link_index)).map_err(refused)?;

    // Snapshotted rather than recomputed, because step 6 overwrites it in place:
    // taking it later makes every delta empty.
    ...
    let check = check_ledger(&CheckOptions {
        ...
        // somebody handed this run a different `--link-index` than the one the
        // ledger records. The other half — the map this run's own extractor is
        // about to rewrite — is the check after the rounds.
        link_index: Some(options.link_index),
        // The same map step 7 renders with, so "the pages were rendered against
        // this" is one value rather than two derivations of one.
    ...
    while !round_in.is_empty() || (rounds == 0 && !check.removed.is_empty()) {
        ...
        // Deletions belong to the first round. The condition is documentation
        // rather than protection: both stages filter the list to modules the base
        // index still holds, so passing it again would be a no-op.
        let deletions =
        ...
        // each name, which the merge is about to overwrite.
        //
        // `--exclude` is the round's memory of what earlier rounds already took,
        // and it is **currently unobservable**: `ownership` excludes its own
        // `--inc` and `--removed` sets already, and a round after the first
        // watches nothing, because a module reaches round 2 through its
        // *references* and its own declaration names are still the base IR's.
        // `tests/incremental.rs` asserts that rather than assuming it, since the
        // argument rests on a fact about Lean and not about this file.
        ...
        // The removals are folded into the merge, so the IR is never left in a
        // state where a deleted module is still indexed.
        let at = Instant::now();
        let merged = run_merge(&MergeOptions {
            ...
            // `index.json` comes out in. Without it a round that adds a module
            // leaves an index a full run would have written differently — same
            // entries, different sequence — and no page byte says so.
            modules: Some(&options.modules),
    ...
    // teardown stays inside `totalSeconds` either way.
    extractor.release();
    write_lines(&work.seen, &seen)?;
    ...
    // A moved render key overrides `--mode` rather than widening it, and **so
    // does a dependency map this run rewrote**: `detect` compared the map as it
    // stood at the head of the run, and the extractor writes it, so the map the
    // renderer is about to read may not be the one `detect` saw. The comparison
    // happens twice, at the two moments its input exists.
    //
    ...
    // UTF-16 code unit order, which every other module list in this project is
    // in. Nothing generated depends on it — the renderer is handed a
    // [`ModuleSet::These`], which is a set — so the order reaches
    // `render-set.txt`, a diagnostic, and stops there.
    ...
    // `ModuleSet::These` of an empty set already renders nothing, so this skip is
    // an optimisation — it saves reading the whole IR to write no file — and not
    // a guard. `tests/incremental.rs` asserts both halves.
    ...
    Ok(Run {
        ...
        timings: {
            ...
            // duration is the gap to the one before it.
            let [
                detect,
                rounds_gap,
                prune_gap,
...
/// Each cumulative mark minus the one before it, in seconds; the first is the
/// gap from zero.
...
/// The wall-clock split of one run.
///
...
///
/// The signature is the guard. [`PruneOptions::ir`] turns on the orphan rule,
...
pub fn incremental(args: &[String]) -> Result<(), Failure> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            // Without it `--link-index` names an input; with it the resident
            // extractor **writes** that file out of the environment it has
            // imported anyway, and the file is an output this run produces before
            // it renders against it.
            ...
            // **A flag of neither extraction path**, and not `--target`: this is
            // the package whose lake-manifest.json and toolchain pin the
            // dependencies, which is a question about the *pages* and is asked
            // however the extraction is done (on a real package the two are the
            // same directory). Left out, dependency links stay relative and the
            // run says so; the ledger that licenses those pages has to have been
            // built the same way, which is why `ledger` takes the same flag.
            "--root" => package = Some(args.value("--root")?.into()),
            // This command renders *and* compares the render key with the
            // ledger's, so it needs the map `build` wrote for both: without it
            // the round's pages lose the links the rest of the tree has, and the
            // key says every page has to be re-rendered — for ever, since the run
            // that re-renders them writes no map either. Read, not resolved:
            // nothing here touches the network.
            "--deps-docs-map" => deps_docs_map = Some(args.value("--deps-docs-map")?.into()),
            // **A flag of `--serve` only**: the resident server's job count is its
            // start-up `cfg` (`Extract.lean:2751`), so it is the pipeline's to
            // choose exactly when the pipeline is the thing that starts it.
            // Behind `--extractor` the parallelism belongs to the extraction
            // program.
    ...
    // **A flag of `--serve` only**, for the same reason `--jobs` is: the map is
    // written by the Lean extractor, and the only extractor whose command line
    // this command spells is the resident one. `--extractor <program>`'s contract
    // is three flags (`--modules --ir-dir --timings`) and adding a fourth would
    // break every program written against it.
    ...
    if !serve {
        // A list and not a `match` on the name with a `_` arm: adding a fourth
        // flag and forgetting the arm makes the `_` answer for it, so the refusal
        // names the new flag while the test behind it reads `--lake` — and the
        // existing flags go on working, so nothing fails.
        for (flag, given) in [
        ...
        if jobs != 1 {
            return usage(
                "--jobs is a flag of --serve: parallelism is the extractor's, and a resident one \
                 fixes it at start-up. Behind --extractor, pass it through with `--extractor-arg \
    ...
    let mode = match mode {
        // **Not** the sound bound — `impact`'s own default is `importers`.
        None => Mode::SelfOnly,
        Some(text) => match Mode::parse(&text) {
            // Refused here rather than carried into `impact`, which only looks at
            // `--mode` when there is something to select: a misspelled mode with
            // an empty changed set would otherwise exit 0 having rendered
            // nothing.
    ...
    // render key and step 7 renders with.
    ...
    // oleans, and every later check is against this one reading.
    ...
    // Not a `?` above: the release has to happen on the failing path too, and
    // while dropping `extractor` would do it silently, doing it here is what puts
    // the stop **before** the error reaches the caller rather than after. `Drop`
    // remains the backstop for a panic.
    extractor.release();
    let run = outcome?;
...
pub(crate) fn serve_options(request: ServeRequest<'_>) -> Result<Serve, Failure> {
    ...
    // exception: a name looked up on PATH, not a path. The map is refused inside
    // the target for the same reason `--ir-dir` is — the package being documented
    // is opened read-only, and 8.5 MB landing in somebody's working tree is a
    // write.
    ...
    // that has to compare equal across runs. `None` when there is no map: the
    // extractor is passed the token only beside `--link-index`.
...
/// checked.
///
/// **The ledger's bytes do not depend on this and its speed does**, which is why
...
/// ```
///
/// **7.4x from one to two** is more than two threads of CPU can explain: the
/// work is reading mmap'd oleans, so the second thread hides page-fault latency
/// rather than adding arithmetic. The ceiling is therefore low — 4 is already
/// within 30% of 8 — so this clamps rather than taking every core:
...
///
/// The map is a function of three inputs (`Extract.lean`'s `writeLinkIndex`):
/// the imported module set, those modules' oleans, and the omit list. The
/// extractor checks the first itself, out of the environment it is holding; this
/// covers the other two.
///
/// **The oleans are covered by three of `extractKey`'s five values rather than
/// by hashing them**: `leanToolchain`, `manifestSha256` and `extractor`, which
/// are this pipeline's existing answer to "could the bytes Lean produces have
/// moved" plus the name of the code doing the writing. **The other two are
/// deliberately left out**: `irSchemaVersion` and `irGenerator` describe **the
...
/// of the map is *not* here: the extractor checks that against its own `#lidx2`
/// marker, where a format change and the check that catches it sit in one file.
///
/// The two parameters are the *package* whose toolchain and manifest pin the
/// dependencies and the *omit list* the map was built against. Swapped, the token
/// is a different string and the map is rebuilt for nothing.
fn link_index_key(package: &Path, omit_list: &Path) -> Result<String, Failure> {
    // `None`: the IR half of the key is not wanted here, so there is nothing to
    // read `<ir>/index.json` for.
...
/// [`REV_HEX_DIGITS`] **lower-case** hex digits: the acceptance oracle
/// normalises `/blob/[0-9a-f]{40}/` and nothing else.
///
/// **The only copy of this decision.** `packages::is_revision` — the one that
/// reads `lake-manifest.json`'s `rev` and `core_githash` — defers here, because
/// asking a second way is how the same forty-digit string became a usage error
/// on one path and a blob base on the other: `is_ascii_hexdigit` is true of
/// `A`-`F`.
#[must_use]
...
/// **Checked here and nowhere else**: `render` and `site` ask only for a
/// non-empty string, because they are called by hand and by harnesses that pass
/// placeholder URLs on purpose. This is the path that runs on *every commit*, and
/// the only place a real revision enters.
...
/// guessed at. The four nested records are read from fixed paths rather than
/// from a glob, so a directory listing's order cannot reach the record.
fn write_timings(
    path: &Path,
    work: &Path,
    summary: &Summary,
...
///
/// `--lib` repeats, and with none given the names are read out of the package's
/// lakefile: a package may declare more than one library, and a caller may want a
/// subset of what it declares.
///
...
/// and the diagnostic files and stops there.
...
/// `detect`, the extractor and `merge`**, and two derivations of "the same" list
...
pub(crate) fn module_names(root: &Path, libs: &[String]) -> Result<Vec<String>, Failure> {
    ...
    // imported anything. The identity for every name that is already an
    // identifier, which is all 432 of the measurement target's.
    let mut names: Vec<String> = paths
        ...
        .collect();
    // `dedup` after the sort: two `--lib` arguments that overlap name the same
    // module twice, and a ledger with a repeated module is one whose `check`
...
///
/// Symlinks are not followed: a symlinked directory inside a library would let
/// one module be listed twice under two names, and the second one has no olean.
#[expect(
...
/// One name per line, and **no line at all** when there are no names: an empty
/// set has to be an empty file rather than one blank line, or `--only-from` and
/// the round loop disagree about what "nothing" is.
...
mod tests {
    ...
    /// The clock is monotonic, so this is a shape the type allows and the run
    /// does not produce — `saturating_sub` is what keeps the two apart.
    #[test]
