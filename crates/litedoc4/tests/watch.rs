...
//!
//! The loop's judgement and the question it asks are pure functions asserted
//! beside them in `crates/litedoc4/src/watch.rs`. What neither can reach is what
//! only exists in a process: the banner, the server thread, the order the lines
//! come out in, and the three things a pass does — announce, build, and say what
//! the build cost. `run_loop` has no exit condition, so there is no way to call
//! it and come back.
//!
//! `run_loop` could be decomposed — a clock and a "build this" closure would
//! make its four arms callable — and deliberately is not, for the reason
//! `watch.rs` gives for refusing a file-system watcher: one path, not two. A
//! seam that lets a test drive the loop without a build is a second definition
//! of what a pass does, and a disagreement between the two reads as a loop that
//! a test says rebuilds and a user says does not. Every arm prints a line
//! instead, and every line is asserted below.
//!
//! `--port 0` is refused by the product on purpose ("an address nobody can
//! type"), so a test cannot ask the kernel for a free port. 18484-18486 is
//! `watch`'s own default (8484) plus 10000: below every ephemeral range (Linux's
//! 32768-60999, macOS's 49152-65535) so the kernel will not have handed one out,
//! clear of `tools/watch-gate.sh`'s 8485, and registered to nothing. One per
//! case, because the cases run on their own threads.
...
/// Pinned rather than derived from a checkout: deriving it is the subject of
/// `tests/build.rs`, not this one's.
const URL: &str = "https://github.com/owner/repo/blob/0123456789abcdef0123456789abcdef01234567";

/// Above the product's 100 ms floor, and small enough that a case waiting for
/// three passes waits under a second.
const INTERVAL_MS: &str = "200";

/// A **bound, not an expectation**: three modules through a `/bin/sh` extractor
/// is milliseconds, and nothing here asserts on a duration.
...
/// What `watch` reads: the lakefile it takes the library from when `--lib` is
...
impl Live {
    ...
    /// Both streams share one file descriptor, so the log is the lines in the
    /// order they were written.
    ///
    /// `--lib` is deliberately absent: the loop reads the library out of the
    /// lakefile and pins it, and the line saying so is asserted below.
    ...
    /// Missing is empty: the file exists from the moment the child is spawned,
    /// but a reader can get there first.
    fn log(&self) -> String {
        fs::read_to_string(&self.log).unwrap_or_default()
    }

    /// "Module M changed", injected into the ledger with the olean left exactly
    /// as it is — which is what makes this expressible without a Lean toolchain.
...
impl Watching {
    /// A dead child is reported as a dead child: a wait that could only time out
    /// would turn "the port was taken" into "sixty seconds passed".
...
/// does, so a failure names the arm. Two of them carry their own reason:
/// `#1 nothing has been built under --out yet` exists because a silent
/// two-minute generation is the shape doc-gen4 #404 was mistaken for, and
/// `waiting one quiet interval` has to fall between the touch and the rebuild —
/// without it the loop would extract into a directory something else is still
/// writing.
#[test]
...
/// Until the first rebuild succeeds there is no site to serve and no state to
/// continue from, so looping would mean printing the same failure every interval
...
fn a_first_rebuild_that_fails_ends_the_command_rather_than_looping_on_it() {
    ...
    // 4 is the extraction stage's own exit for an extractor that failed, and it
    // reaches the shell unchanged rather than being swallowed by the loop.
    assert_eq!(
...
/// A rebuild that retried immediately would start a fresh extraction every
/// interval for as long as the failure lasted — the shape doc-gen4 #404 is named
/// after. What stops it is `Step::Skip`, the same answer the last pass already
/// acted on, so the assertion is a **count** and not a presence.
#[test]
fn a_later_rebuild_that_fails_is_reported_once_and_not_retried_every_interval() {
    ...
    // Breaking the *world* rather than passing `--fail` keeps the failure to one
    // module, which is what a real broken build looks like.
    ...
    // Six intervals; a loop that retried would be on `#8` by now. The count is
    // taken over the line the *pass* prints, because one that retried and
    // reported only once would still have re-run the extractor.
    ...
    // The other half of "wait for the world to move": a loop that waited for
    // ever would be indistinguishable from a hang.
